...
interface ViewApprovalGroupProps {
  open: boolean;
  data: object;
  closePopup: () => void;
}

const ViewApprovalGroup = ({
  closePopup,
  open,
  data,
}: ViewApprovalGroupProps) => {
  const approvalGroupDetails = Object.entries(data).map(([key, value]) => ({
    title: key,
  ...
  return (
    <CustomDialog
      ...
      title="View Approval Group Details"
      size="md"
      footer={
        ...
        <div className="bg-white border rounded-lg shadow p-4">
          <div className="max-h-[400px] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
              {approvalGroupDetails.map((item, index) => (
                <div
                  key={index}
