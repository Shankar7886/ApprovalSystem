...
interface ViewProjectLocationProps {
  open: boolean;
  data: object;
  closePopup: () => void;
}
...
const ViewProjectLocation = ({
  ...
  open,
  data,
}: ViewProjectLocationProps) => {
  const projectLocationDetails = Object.entries(data).map(([key, value]) => ({
    title: key,
  ...
  return (
    <CustomDialog
      ...
      title="View Project Location Details"
      size="md"
      footer={
        ...
        <div className="bg-white border rounded-lg shadow p-4">
          <div className="max-h-[400px] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
              {projectLocationDetails.map((item, index) => (
                <div
                  key={index}
...
export default ViewProjectLocation;
