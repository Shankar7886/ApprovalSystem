...
interface ViewFormGroupProps {
  open: boolean;
  data: object;
  closePopup: () => void;
}

const ViewMenuGroup = ({ closePopup, open, data }: ViewFormGroupProps) => {
  const itemMasterDetails = Object.entries(data).map(([key, value]) => ({
    title: key,
        ...
        <div className="bg-white border rounded-lg shadow p-4">
          <div className="max-h-[400px] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
              {itemMasterDetails.map((item, index) => (
                <div
                  key={index}
